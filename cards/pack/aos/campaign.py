...
def PrepareEvidenceEnvelopes() -> 'Ability':
    def action(effect: 'Effect', message: 'Message.WhenCampaignSetup') -> None:
        ...
        # Gained evidence is campaign information, not a card in play or in a
        # deck. Keep it removed until its Setup instruction resolves below.
    ...
    def action(effect: 'Effect', message: 'Message.WhenCampaignSetup') -> None:
        ...
        for name, linked_ids in BOARD_MEMBERS.items():
            ...
            if _campaign_flag(f"{name} Flipped", effect):
                # Restore counters while the card is out of play so restoring
                # campaign state does not fire its attachment response.
                if secrets:
...
def ResolveCampaignVictory(level: int) -> 'Ability':
    def action(effect: 'Effect', message: 'Message.WhenGameOver') -> None:
        ...
        if level <= 4:
            for name in BOARD_MEMBERS:
                ...
                if not member:
                    # The attachment side has a different printed name, so
                    # identify it through its linked environment face.
                    member = next((
        ...
        # A campaign win earns one random card from the S.H.I.E.L.D.
        # envelope when at least one unflipped member is fully investigated.
        investigated = any(
        ...
        if not shield_envelope.initialize:
            return
        candidates = shield_envelope.deck.FindCards(card_type=Evidence)
        if not candidates:
            return
    ...
    # Forced-response priority runs after rule/challenge win modifiers but the
    # message still resolves before the engine renders its victory panel.
    return Ability(
